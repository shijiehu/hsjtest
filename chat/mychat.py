...
class Server():
    ...
    def listen_client(self):
        while 1:
            ...
            header,data = topInfo.decode('iso-8859-1').split('\r\n\r\n', 1)
            ...
            res_key = base64.b64encode(hashlib.sha1((sec_key + config['MAGIC_STRING']).encode('utf-8')).digest()).decode('utf-8')

            str_handshake = config['HANDSHAKE_STRING'].replace('{1}', res_key).replace('{2}',config['HOST'] + ':' + str(config['PORT']))
            tcpClientSock.send(str_handshake.encode('utf-8'))
    ...
    def send_data(self, data, address):
        ...
        try:
            username = parse.unquote(self.users[address])
        except:
            username = '匿名用户'
        if data:
